...
void changeData(Node* head) {
	SplittedInfo* info = createNewSplittedInfo();
	printf("Enter the class to change information about: \n");
	...
	printf("Please, enter cipher, what data you want to change: \n");
	...
	while (true) {
		...
		char* choice = (char*)malloc(sizeof(char) * 5);
		scanf("%s", choice);
		...
		switch(atoi(choice)) {
			...
			case 6:
				...
				return;
			default:
				printf("Saving data and exit\n");
				printf("New data:\n");
				printSplittedInfo(info);
				free(findByClass(head, buf)->string);
